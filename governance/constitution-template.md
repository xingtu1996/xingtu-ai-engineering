***REMOVED*** 宪法模板（Constitution / Governance Baseline）
...
***REMOVED******REMOVED*** 二、示例企业宪法骨架（引用不复制）

***REMOVED******REMOVED******REMOVED*** 铁律（三条）
| ***REMOVED*** | 铁律 | 一句话 |
|---|------|--------|
...
***REMOVED******REMOVED******REMOVED*** RED 红线（示例，按项目增删）
| ***REMOVED*** | 红线 |
|---|------|
| RED-1 | 禁 `rm -rf`（任何场景） |
...
***REMOVED******REMOVED******REMOVED*** 禁止模式（≥10 条，每条给替代）
空 catch→log.error｜SQL `${}`→`***REMOVED***{}`｜事务内远程调用→事务外/MQ｜消费者不幂等→幂等键｜无分页→强制分页｜密钥明文→环境变量｜日志敏感明文→脱敏｜循环内远程调用→批量｜无回滚→回滚方案｜硬编码容量→标设计寿命

***REMOVED******REMOVED******REMOVED*** 安全三条件（计费/库存/权限必填）
退出（可中止）｜幂等（重入安全）｜回滚（失败可恢复）

***REMOVED******REMOVED******REMOVED*** 质量门禁（提交前）
宪法检查｜级联影响｜测试金字塔 L1≥85% / L2 curl≥3｜对抗审查

***REMOVED******REMOVED*** 三、积木化宪法（按阶段裁剪）
