...
***REMOVED******REMOVED*** 会话复盘：{主题}（{日期} 会话）

***REMOVED******REMOVED******REMOVED*** 一、会话背景
- 主题 / Ticket / 关联分支 / session 原始文件（`~/.claude/projects/{项目}/*.jsonl`，便于溯源）

***REMOVED******REMOVED******REMOVED*** 二、本次完成（含提交/合并）

| 项 | 内容 | 提交/合并 |
|----|------|----------|

***REMOVED******REMOVED******REMOVED*** 三、关键文件/资产
- 改动的代码路径 + 关键决策（一句话）

***REMOVED******REMOVED******REMOVED*** 四、沉淀资产
- 写入 memory/incidents/rules/specs 的条目（链接）

***REMOVED******REMOVED******REMOVED*** 五、待办/断点续传
- 下一步做什么（+ 继续会话提示词要点）
