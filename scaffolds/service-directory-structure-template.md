***REMOVED*** 服务目录结构样板（新服务脚手架）

> **用途**：新服务脚手架，AI 有样学样生成目录+创建步骤。
> 基准：`CLAUDE.md`「Typical DC Service」+ `java-rules.md` §二/§三。

***REMOVED******REMOVED*** 一、目录结构树

```
dc-<service>/
├── src/main/java/com/示例企业/ctf/<service>/
│   ├── controller/      ***REMOVED*** 收参→校验→调 application
│   ├── application/     ***REMOVED*** 业务编排+事务（核心层）
│   ├── domain/          ***REMOVED*** 领域对象+Mapper 接口
│   ├── client/          ***REMOVED*** Feign+fallback 降级
│   ├── query/           ***REMOVED*** CQRS 查询层
│   ├── listener/        ***REMOVED*** MQ 监听→委托 application
│   ├── infrastructure/  ***REMOVED*** DAO/Mapper 实现
│   └── utils/           ***REMOVED*** 工具类
├── src/main/resources/
│   ├── bootstrap.yml        ***REMOVED*** 默认 local
│   ├── bootstrap-dev.yml    ***REMOVED*** 开发
│   ├── bootstrap-qa.yml     ***REMOVED*** QA
│   └── bootstrap-remote.yml ***REMOVED*** 生产
└── pom.xml              ***REMOVED*** parent=ctf-parent/adpt-parent
```

***REMOVED******REMOVED*** 二、DDD 分层职责（java-rules.md §二）
