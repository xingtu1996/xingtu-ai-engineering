***REMOVED*** 方法：CLI 封装（CLI Encapsulation / Operation Twin Interface）
...
***REMOVED******REMOVED*** CLI vs Skill vs MCP（三接口定位）
...
***REMOVED******REMOVED*** 最小示例（可抄，按此骨架扩展）

```bash
***REMOVED***!/usr/bin/env bash
***REMOVED*** mytool.sh — 动词式子命令 + --json + --dry-run + 幂等键
set -euo pipefail
...
emit() { ***REMOVED*** 结构化输出：语义层契约
  [ "$JSON" = "1" ] && echo "{\"status\":\"$1\",\"decision\":\"$2\",\"action\":\"$3\",\"payload\":\"$4\"}" || echo "$1 | $2 | $3 | $4"
}
case "$CMD" in
  list)
    ***REMOVED*** 只读操作默认不写（多读少写）
    emit ok view list "查询 10 条记录"
    ;;
  deploy)
    KEY="deploy-$(git rev-parse --short HEAD 2>/dev/null || echo manual)"  ***REMOVED*** 幂等键
